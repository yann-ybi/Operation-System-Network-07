 ...
 |																			|
 |	Created by Jean-Yves Hervé on 2018-12-05 (CPP version)					|
 |	Revised 2023-04-27														|
 +-------------------------------------------------------------------------*/ 
//
//  main.cpp
...
// duration after which the program ends (in seconds)
int duration = 10;
int elapsed = 0;
...
int main(int argc, char** argv)
{
	if (argc != 6)
	{
		std::cerr << "Usage " << argv[0] << "<num_cols> <num_rows> <num_boxes> <num_doors> <timer>\n";
		return 1;
	}
...
/**
 * Sleeps for the duration passed as argument as long as the program is not being quit with escape
 * then timeOut is set to true and program ends
 * I made a thread to call that function
 */
void timeoutFunc(int totalSleep)
{
    while (elapsed < totalSleep && !quitting)
    {
...
{
	std::string fileName = "robotSimulOut " + std::to_string(numSim) + ".txt";
...
void initializeApplication(void)
{
	...
	threads.emplace_back(timeoutFunc, duration);
}
...
void moveRobot(RobotInfo* robot, char dir, int dist)
{
    ...
    for (int k = 0; k < dist; k++)
    {
        if (robot->isPushing)
        {
			while (true)
			{
				if (!robot->isAlive || isTimeout) return;
