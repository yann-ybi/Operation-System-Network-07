...
void displayStatePane(void)
{
	...
	int numMessages = 4;
	snprintf(message[0], 30, "Times out in %d s !",  duration - elapsed);
	snprintf(message[1], 30, "I like cheese");
	snprintf(message[2], 30, "System time is %ld", time(NULL));
	snprintf(message[3], 30, "We have %d doors", numDoors);
						...
						void (*stateDisplayCB)(void))
{
	...
	std::string fileName = "Box Pushing #" + std::to_string(numSim) + " -- CSC 412 - Spring 2023";
	gMainWindow = glutCreateWindow(fileName.c_str());
